...
글을 쓰기 전에 **생각 담당 모델**이 먼저 돈다 (`BRAIN_MODEL` · Groq 키가 있으면 기본 gpt-oss-120b). 제품 정보와 참고 자료를 읽고 한 줄 결론 · 알맹이 · 숫자 · 고를 때 기준 · 확인할 점을 JSON 으로 정리하고 글쓴이는 그 답을 글로 옮긴다. 자료에 없는 것은 넣지 못하게 출처를 붙인다. 생각 담당이 실패하면 글쓴이가 예전처럼 자료를 직접 읽는다. 모델은 둘이고 시스템은 하나다.
