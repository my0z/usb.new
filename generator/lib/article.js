...
- intro_html 첫 문장은 제품을 바로 소개하지 않는다. "이 제품을 써보니" 대신 이 종류 제품이 흔히 해결하는 불편한 상황을 먼저 그린다. 예: "보조배터리를 챙겼는데 막상 꺼내보면 방전돼 있던 경험이 있을 것이다." 그 상황은 제품 정보에 있는 사실 안에서만 만든다.
- 사양을 나열하지 않는다. 제품 정보와 참고 자료에서 확인되는 것 중 구매를 결정짓는 두 가지만 골라 본문 앞쪽에서 짚는다. 나머지는 specs 표에만 남긴다.
- 숫자와 전문 용어는 일상 말로 풀어 옮긴다. "20000mAh" 라면 "스마트폰을 서너 번 채우는 양" 처럼 무엇이 편해지는지로 바꾼다. specs 표의 값은 원래 표현 그대로 둔다.
- sections 중 한 곳에는 이 제품을 산 뒤 실제로 쓰는 하루 속 장면을 그린다. 제품 정보에 있는 기능만으로 장면을 구성하고 없는 기능을 상상해 넣지 않는다.
- outro_html은 사라고 강요하지 않는다. "구매 링크는 아래에 있습니다" 같은 문장 대신 독자가 자신의 상황과 견줘보고 싶어지는 질문이나 장면으로 끝맺는다.
...
출력 JSON 형식:
{
  ...
  "specs": [{"k": "항목", "v": "값"}],
  "intro_html": "<p>..</p><p>..</p>  (2문단. 불편한 상황에서 시작해 이 제품이 눈에 띄는 이유로 이어진다)",
  "sections": [
  ...
  "outro_html": "<p>누구에게 맞는지 정리하고 강요 없이 견줘볼 질문으로 끝맺는다</p>",
  "faq": [
    {"q": "질문", "a": "답변 한두 문장"},
    {"q": "질문", "a": "답변 한두 문장"},
    {"q": "질문", "a": "답변 한두 문장"}
...
export function buildPrompt({ keyword, query, intent = '', hint = '', products, facts = [], brief = '' }) {
...
노리는 검색어 (제목 앞에 넣는다): ${intent || keyword}${hint ? `\n검색 의도: ${hint}` : ''}
검색어: ${query}
제품 목록 (1번이 주인공):
${lines.join('\n')}
${factBlock}${brief}
