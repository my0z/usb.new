...
export function notFoundPage({ canonical, recent = [] }) {
  const body = html`<div class="shell shell--narrow">
    <header class="page-head reveal">
      <p class="eyebrow">404</p>
      <h1 class="page-title">찾는 페이지가 없다</h1>
      <p class="page-desc">주소가 바뀌었거나 삭제된 글일 수 있다. 최근 글부터 확인해 보자.</p>
    </header>
  </div>
  ${recent.length ? html`<div class="shell">${cardGrid(recent.slice(0, 3), { variant: 'compact' })}</div>` : ''}`;
