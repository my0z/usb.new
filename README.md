...
며칠간 조금씩 상승하다가 한 번 크게 튀어 오르고 어느 정도 유지되다가 하루이틀 사이 20% 이상 급락한 뒤 다시 그 수준에서 유지되는 종목을 과거 일봉 데이터에서 찾아주는 도구입니다. 실시간 감시와 별개로 필요할 때 수동으로 실행합니다.

```
python scan_pattern.py                  # WATCHLIST + 당일 거래량 상위 종목 대상
python scan_pattern.py 005930 000660    # 특정 종목만 지정
```

패턴 조건(상승폭 유지기간 급락폭 등)은 `config.py`의 `RISE_*` `SPIKE_*` `PLATEAU_*` `CRASH_*` `POST_CRASH_*` 값으로 조정합니다.

이 도구는 과거에 이런 모양이 있었던 종목을 찾아줄 뿐 앞으로 같은 패턴이 반복된다고 보장하지 않으며 매수 매도를 추천하지 않습니다.
