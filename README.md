...
`.env`에 `PROFILE=aggressive`를 넣으면 아래 값이 한꺼번에 바뀝니다.

| 항목 | standard | aggressive |
|---|---|---|
| 스크리닝 종목 수 | 20 | 40 |
| 실시간 구독 상한 | 40 | 80 |
| 재스크리닝 주기 | 30분 | 10분 |
...
장 시작 직후에는 당일 거래량 순위가 거의 비어 있어서 봇이 정해진 주기마다 다시 스크리닝해 새로 급증한 종목을 감시 목록에 추가합니다. 15시 이후에는 재스크리닝을 멈춥니다.

공격적으로 갈수록 신호가 많아지고 손익 폭도 커집니다. 하루 100만원을 벌 가능성만큼 하루 100만원을 잃을 가능성도 같이 커진다는 점을 감안하고 쓰세요.
